...
function App() {
  ...
  새로운 참조값을 할당한다. 즉 참조값이 계속 바뀌므로 useEffect가 지속적으로
  실행된다. JS에서 함수는 객체형이기 때문이다.
  */
  // const somthingGood = () => {
  //   console.log(`somthingGood호출 : ${countNumber}, ${randomNumber}`);
  //   return;
  // };
  ...
  Step2 : 함수에 useCallback을 적용하여 렌더링 시 딱 한번만 함수를 캐시에
        ...
        변경을 감지하지 못한다. 최초 실행시 초기값 0만 출력된다.

  Step3 : countNumber가 변경될떄마다 새롭게 메모이제이션 하므로 변경된 State를
