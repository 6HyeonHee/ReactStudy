...
function App() {
  ...
  // 도큐먼트 수정을 위한 함수(쓰기와 동일함. 내용이 바뀌면 자동으로 수정처리)
  const memberEdit = async (p_collection, p_id, p_pass, p_name) => {
    /*
    doc(파이어스토어, 컬렉션명, 도큐먼트명), { 수정할 내용을 JSON 객체로 기술 }
    */
  ...
  useEffect(() => {
    ...
    getCollection().then((result) => {
      ...
      // State를 벼경하여 <select>태그에 <option>을 추가한다.
      setShowData(result);
    });
  }, []);

  // <input>에 설정된 값은 State를 통해 변경해야 하므로 갯수만큼 추가한다.
  ...
  return (
    <div className="App">
      ...
      <h3>개별조회 및 수정하기</h3>
      {/* 항목 하나를 선택하면 change이벤트가 발생한다. */}
      <select
        onChange={async (e) => {
          // 선택항목의 value 즉 아이디를 얻어온다.
